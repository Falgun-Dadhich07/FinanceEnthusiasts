...
const sparklineData = [
    14500, 15000, 15400, 15800, 16000, 16500, 17500, 18200, 18900, 19400, 19800, 20500, 21000, 21200, 22550,
];
...
//     { value: 22550 },
// ];
function generateSparkPath(values, progress, width = 100, height = 15) {
    const visiblePoints = Math.max(
        1,
        Math.floor(values.length * progress)
    );

    const slice = values.slice(0, visiblePoints);
...
export function MarketIntelligence() {
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            setAnimatedRevenue(Math.floor(targetRevenue * progress));
    ...
    const progress = animatedRevenue / targetRevenue;

    const sparkPath = useMemo(
        () => generateSparkPath(sparklineData, progress),
        [progress]
    );
                    ...
                    <div className="bg-[#7FB2D5]/5 backdrop-blur-xl border border-[#7FB2D5]/10 rounded-lg p-6 relative overflow-hidden hover:border-[#7FB2D5]/30 transition-all duration-300 h-full">
                        ...
                        <div className="relative z-10">
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-xs text-[#7FB2D5]/60 tracking-widest">
                                    REVENUE
                                </span>
                            ...
                            {/* SVG Sparkline (SYNCED WITH NUMBER) */}
                            ...
                            {/* Data points indicator (unchanged) */}
                            <div className="flex gap-1 mt-2">
                                {sparklineData.map((_, i) => (
                                    <div
                                        key={i}
                                        className="flex-1 h-1 bg-[#7FB2D5]/20 rounded"
                                    />
                                ))}
                            </div>
                        </div>
